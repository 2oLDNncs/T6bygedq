...
public final class CblTemplateMaker extends JavaTemplate {
	...
	protected final void printMemberMethods() {
		...
		while (this.inputLines.hasNext()) {
			final var line = this.inputLines.next();
			var m = P_IDENTIFICATION_DIVISION.matcher(line);
			
			if (m.matches()) {
				if (!M_printIdentificationDivision.equals(this.currentMethod)) {
					this.subpgmNum += 1;
					final var subpgmClassName = String.format("%s_%s", this.generatedClassName, this.subpgmNum);
					
					this.inputLines.previous();
					final var subpgmMaker = new CblTemplateMaker(subpgmClassName, this.inputLines);
